...
**數字使用原則：下方 TAIFEX 資料為權威來源，禁止更改或臆測這些數字。國際盤部分可用 Google Search 查詢，但若查不到明確數字請如實說明，不可捏造。**

**TAIFEX 資料（{td}，此為本報告唯一量化基礎）**
...
**要求**
1. 用繁體中文、**800–1200 字**、HTML 格式（可用 <h3>、<h4>、<p>、<ul>、<li>、<strong>、<table> 基本標籤；不要用 <html><body> 包外層、不要用 markdown 語法）
2. 結構：
   - **<h3>夜盤速覽</h3>**：3–4 句濃縮（夜盤漲跌幅、缺口方向、成交量熱度，數字必須與上方 TAIFEX 資料一致）
   - **<h3>國際盤脈絡</h3>**：用 Google Search 查 {td} 當晚美股（S&P 500、那斯達克、費城半導體）、原油、黃金、美債殖利率、台積電 ADR 的重點變化；150–250 字；若查詢結果不確定請寫「數據待確認」，不捏造數字

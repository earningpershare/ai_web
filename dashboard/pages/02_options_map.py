...
# detect available contract months
all_months_df = fetch("/options/strike-cost", {"trade_date": selected_date})
available_months = sorted(all_months_df["contract_month"].unique().tolist()) if not all_months_df.empty else []

w_months = [m for m in available_months if "W" in m]
f_months = [m for m in available_months if "F" in m and "W" not in m]
monthly  = [m for m in available_months if "W" not in m and "F" not in m]

default_month = w_months[0] if w_months else (available_months[0] if available_months else "")
selected_month = st.sidebar.selectbox(
    "到期月份（主顯示）", available_months,
